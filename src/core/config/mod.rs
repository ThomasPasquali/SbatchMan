...
pub fn sbatchman_init(path: &PathBuf) -> Result<(), ConfigError> {
  ...
  confy::store_path(path.join("sbatchman.conf"), config).map_err(|e| ConfigError::ConfigError(e.to_string()))?;
  Ok(())
}

pub fn set_cluster_name(path: &PathBuf, name: &str) -> Result<(), ConfigError> {
  let mut config: SbatchmanConfig = confy::load_path(path.join("sbatchman.conf")).map_err(|e| ConfigError::ConfigError(e.to_string()))?;
...
pub fn get_cluster_name(path: &PathBuf) -> Result<String, ConfigError> {
  let config: SbatchmanConfig = confy::load_path(path.join("sbatchman.conf")).map_err(|e| ConfigError::ConfigError(e.to_string()))?;
