...
use ratatui::{
  ...
  text::{Line, Span, Text},
  widgets::{
    Block, Borders, Cell, List, ListItem, ListState, Paragraph, Row, Table, TableState, Tabs, Wrap,
  },
};
...
impl App {
  ...
  fn get_filtered_jobs(&self, tab: JobTab) -> Vec<&Job> {
    self
      ...
      .filter(|job| {
        ...
        // Apply status filter
        if !self.job_filter.statuses.is_empty() && !self.job_filter.statuses.contains(&job.status) {
          return false;
        }
    ...
    match &self.mode {
      AppMode::JobMonitoring(tab) => {
        ...
        if self.show_actions_popup && !self.show_confirmation_popup {
          ...
          match mouse.kind {
            MouseEventKind::ScrollDown => match self.action_target {
            ...
            MouseEventKind::ScrollUp => match self.action_target {
              ActionTarget::Selected => {
                let i = self.selected_action_list_state.selected().unwrap_or(0);
                self
                  .selected_action_list_state
          ...
          match mouse.kind {
            MouseEventKind::ScrollDown => {
              ...
              if !jobs.is_empty() {
                ...
                self
                  .job_table_state
                  .select(Some((i + 1).min(jobs.len() - 1)));
              }
            }
      ...
      AppMode::LogViewer => match mouse.kind {
        MouseEventKind::ScrollDown => {
      ...
      AppMode::ScriptViewer => match mouse.kind {
        MouseEventKind::ScrollDown => {
      ...
      AppMode::FilterConfig(section) => {
        ...
        match mouse.kind {
          MouseEventKind::ScrollDown => match current_section {
          ...
          MouseEventKind::ScrollUp => match current_section {
            FilterSection::Status => {
              let i = self.filter_status_list_state.selected().unwrap_or(0);
              self
                .filter_status_list_state
                .select(Some(i.saturating_sub(1)));
            }
            FilterSection::Config => {
              let i = self.filter_config_list_state.selected().unwrap_or(0);
              self
                .filter_config_list_state
                .select(Some(i.saturating_sub(1)));
            }
          },
          _ => {}
      ...
      AppMode::ColumnConfig => match mouse.kind {
        MouseEventKind::ScrollDown => {
          let i = self.column_list_state.selected().unwrap_or(0);
          self
            .column_list_state
            .select(Some((i + 1).min(self.column_config.columns.len())));
        }
        MouseEventKind::ScrollUp => {
          let i = self.column_list_state.selected().unwrap_or(0);
    ...
    match &self.mode {
      ...
      AppMode::FilterConfig(section) => {
        ...
        match key {
          ...
          KeyCode::Down => match current_section {
            FilterSection::Status => {
              let i = self.filter_status_list_state.selected().unwrap_or(0);
              let max = Status::all_variants().len().saturating_sub(1);
              self.filter_status_list_state.select(Some((i + 1).min(max)));
          ...
          KeyCode::Up => match current_section {
            FilterSection::Status => {
              let i = self.filter_status_list_state.selected().unwrap_or(0);
              self
                .filter_status_list_state
          ...
          KeyCode::Char(' ') | KeyCode::Enter => match current_section {
            FilterSection::Status => {
  ...
  fn draw_job_monitoring(&mut self, f: &mut Frame, tab: JobTab) {
    ...
    let headers = self
      ...
      .columns
      .iter()
      .map(|c| Cell::from(c.name()).style(Style::default().fg(Color::Yellow)));
        ...
        let cells: Vec<Cell> = self
          ...
          .map(|col| {
            match col {
              ...
              ColumnType::Status => Cell::from(format!("{:?}", job.status))
                .style(Style::default().fg(job.status.color())),
              ColumnType::SubmitTime => {
                Cell::from(job.submit_time.map(|t| t.to_string()).unwrap_or_default())
              }
              ColumnType::EndTime => {
                Cell::from(job.end_time.map(|t| t.to_string()).unwrap_or_default())
              }
  ...
  fn draw_config_monitoring(&mut self, f: &mut Frame) {
    ...
    .header(
      Row::new(vec!["ID", "Name", "Cluster ID"])
        .style(Style::default().add_modifier(Modifier::BOLD)),
    )
    .block(
      Block::default()
  ...
  fn draw_filter_config(&mut self, f: &mut Frame, section: FilterSection) {
    ...
    // Status filter list
    let status_items: Vec<ListItem> = Status::all_variants()
      .iter()
      .map(|status| {
        let checked = if self.job_filter.statuses.contains(status) {
          "[x]"
        } else {
          "[ ]"
        };
  ...
  fn draw_actions_popup(&mut self, f: &mut Frame) {
    ...
    if matches!(self.action_target, ActionTarget::Selected) {
      f.render_stateful_widget(
        selected_list,
        chunks[0],
        &mut self.selected_action_list_state,
      );
